...
class ExchangeKeyService(baseService):
    ...
    def decrypt_message(self,encrypt_message_b64,private_key):
        ...
        if correct_decrypt:
            ...
            forVerification = nymi + "||||" + pki + "||||" + pkj + "||||" + kij + "||||" + lij
            print "There is the data " , nymi , pkj , kij , lij , sij
            print "Decypted message " , decrypted_message
            if (signature.verify(forVerification,self.dirc(__file__, "../../../keys" , "/private_key.pem"),sij) ):
                db_client.check_update_nym(nymi,pkj,lij,kij)
        else:
